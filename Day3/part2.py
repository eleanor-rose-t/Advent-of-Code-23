...
def calculate_part_numbers(symbol_with_neighbors_coordinates, lines):
    ...
    return sum(part_numbers), symbol_neighbor_gears
...
def sum_gear_ratios(lines):
    ...
    _, symbol_neighbor_gears = calculate_part_numbers(symbol_with_neighbors_coordinates, lines)
    
    star_coordinates = [(x, y) for (x, y), char in symbol_coordinates.items() if char == "*"]
    
    eligible_star_coordinates = [
        (x, y) for (x, y) in star_coordinates if len(symbol_neighbor_gears[(x, y)]) == 2
    ]
    
    gears = [symbol_neighbor_gears[coord] for coord in eligible_star_coordinates]
    gear_products = [math.prod(gear) for gear in gears]
    
    power_sum = sum(gear_products)

    return power_sum

def main():
    lines = read_input("input.txt")
    answer = sum_gear_ratios(lines)
    print(answer)
